...
class InspectNavigationTest(unittest.TestCase):
    ...
    def test_affect_report_detail_includes_related_memory_item_summaries(self) -> None:
        ...
        self.assertIn("function timelineHref(filters)", html)
        self.assertIn("function memoryItemsHref(filters)", html)
...
class FollowupValidityInspectServiceTest(unittest.TestCase):
    ...
    def test_followup_validity_report_html_groups_all_followup_states_together(self) -> None:
        ...
        self.assertIn("function memoryItemsHref(filters)", html)
        self.assertIn("function timelineHref(record)", html)
        self.assertIn("tailwag-memory-items.html", html)
        self.assertIn("tailwag-person-timeline.html", html)
...
class MemoryItemInspectServiceTest(unittest.TestCase):
    ...
    def test_memory_items_report_includes_distributions_and_html_hash_support(self) -> None:
        ...
        self.assertIn("function renderOverviewSankey()", html)
        self.assertIn("function overviewHrefForNode(label)", html)
        ...
        self.assertIn("function recordMatchesEpisode(record, episodeId)", html)
        self.assertIn("function validityBucket(record)", html)
        self.assertIn("function timelineItemLink(itemId)", html)
...
class PersonTimelineReportTest(unittest.TestCase):
    def test_person_timeline_report_html_has_nav_and_hash_person_filter(self) -> None:
        ...
        self.assertIn("function activePeopleLabel(personIds, visible, selectedItem, selectedHasMemory)", html)
        self.assertIn("function filtersFromHash()", html)
        self.assertIn("function recordMatchesItem(record, itemId)", html)
        self.assertIn("function memoryItemsHrefForRecord(record)", html)
        self.assertIn("tailwag-memory-items.html", html)
...
class InspectPlaceholderFilesTest(unittest.TestCase):
    def test_committed_inspect_placeholders_use_canonical_names(self) -> None:
        ...
        self.assertIn("position: sticky", shared_css)
        self.assertIn(".pill", shared_css)
        self.assertIn(".toolbar button", shared_css)
        shared_js = (inspect_dir / "tailwag-inspect.js").read_text()
        self.assertEqual(shared_js, INSPECT_SHARED_JS)
        self.assertIn("window.inspectFilters", shared_js)
