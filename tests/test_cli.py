...
class CliTest(unittest.TestCase):
    ...
    def test_inspect_affect_html_writes_self_contained_report(self) -> None:
        ...
        self.assertIn("id=\"axisY\"", html)
        self.assertIn("id=\"axisX\"", html)
        self.assertIn("function renderAxes()", html)
        self.assertIn("const origin = valueToScreen(0, 0)", html)
        self.assertIn("axisY.style.left = `${origin.x}px`", html)
        self.assertIn("axisX.style.top = `${origin.y}px`", html)
        self.assertIn("resetZoom.disabled = false;\n      renderTicks();\n      renderPoints();", html)
        self.assertNotIn(".grid::before", html)
        self.assertNotIn(".grid::after", html)
