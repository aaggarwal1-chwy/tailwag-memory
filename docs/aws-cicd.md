...
| Variable | Value |
| --- | --- |
| `AWS_ACCOUNT_ID` | AWS account ID that owns the dev deployment |
| `AWS_ROLE_ARN` | IAM role assumed by GitHub Actions through OIDC |
| `WORKER_CODE_S3_BUCKET` | `aaggarwal1-tailwag-worker-code-<account-id>-us-east-2` |
| `REPORTS_BUCKET_NAME` | `aaggarwal1-tailwag-reports-<account-id>-us-east-2` |
...
| `TAILWAG_ALB_BASE_URL` | unset; skip deployed API smoke checks |
...
| `WORKER_SUBNET_IDS` | unset; omit Lambda VPC config |
| `WORKER_SECURITY_GROUP_IDS` | unset; omit Lambda VPC config |
...
if the AWS account does not already have one.
...
6. Create the AWS OIDC deploy role and set `AWS_ROLE_ARN`.
7. Set `WORKER_SUBNET_IDS` and `WORKER_SECURITY_GROUP_IDS` when Neo4j is private
   in the VPC.
8. Start `Deploy AWS Dev` manually from GitHub Actions.
