...
| AWS Deployment Agent | AWS deployment resources, worker packaging, and cloud infrastructure templates | CloudFormation/SAM templates, IAM examples, packaging helpers, AWS deployment docs |
...
| Need AWS deployment templates, IAM examples, worker packaging, queue/state/report cloud resources, or AWS deployment docs | AWS Deployment Agent | Project Scaffold Agent, Source Adapter Agent, Documentation Agent, Release Quality Gate Agent | Deployment resources only; do not change package API contracts, CLI defaults, or graph behavior | Handoff to Source Adapter Agent for polling behavior gaps and Project Scaffold Agent for package metadata changes |
