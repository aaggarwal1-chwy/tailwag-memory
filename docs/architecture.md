...
Episodes represent stored interactions such as conversations, encounters, and Slack threads. `summary_embedding` and `transcript_embedding` are generated by the configured embedding provider. Raw recordings are not stored.
...
Tailwag provides graph lookups for episodes by person, episodes by place, events by place, and biometric person recognition. Vector retrieval supports episode summary search, episode transcript search, hybrid person/place-filtered episode search, and memory item summary search.

`person_context()` returns one prompt-ready deterministic context surface. It combines active durable memory items, visible follow-ups, and bounded recent episode summaries. When `current_text` or `semantic_scope` is supplied, memory item ranking can use vector similarity, but rendered episode context remains the bounded recent episode lines.
