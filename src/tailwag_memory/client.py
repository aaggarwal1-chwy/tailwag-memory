...
class TailwagMemoryClient:
    ...
    def person_context(self, person_id: str, limit: int = 10) -> str:
        retrieval = PersonContextRetrievalService(self.runner)
        provider = OpenAIPersonContextProvider(
            api_key=self.settings.openai_api_key,
            model=self.settings.synthesis_model,
        )
        return PersonContextSynthesisService(retrieval, provider).context_for_person(person_id, limit=limit)
